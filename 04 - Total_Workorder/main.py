...
# --- 核心配置 ---
# 文件路径配置
OUTPUT_DIR = r"O:\My Drive\071 - SAP 数据\Total_Workorder"
...
def get_date_range():
    ...
    Start_date = 运行年的1月1日，End_date = 运行年的12月31日
    日期格式为 MM/DD/YYYY
...
def get_work_order():
    ...
    try:
        ...
        # 输入订单类型：PM01, PM02, PM03, PM10, ZPM3, ZPM4, ZPM8
        ...
        session.findById("wnd[1]/usr/tabsTAB_STRIP/tabpSIVA/ssubSCREEN_HEADER:SAPLALDB:3010/tblSAPLALDBSINGLE/ctxtRSCSEL_255-SLOW_I[1,3]").text = "PM10"
        session.findById("wnd[1]/usr/tabsTAB_STRIP/tabpSIVA/ssubSCREEN_HEADER:SAPLALDB:3010/tblSAPLALDBSINGLE/ctxtRSCSEL_255-SLOW_I[1,4]").text = "ZPM3"
        session.findById("wnd[1]/usr/tabsTAB_STRIP/tabpSIVA/ssubSCREEN_HEADER:SAPLALDB:3010/tblSAPLALDBSINGLE/ctxtRSCSEL_255-SLOW_I[1,5]").text = "ZPM4"
        ...
        print(f"✅ 工单数据查询完成。日期范围: {start_date} 至 {end_date}，工单类型: PM01, PM02, PM03, PM10, ZPM3, ZPM4, ZPM8")
...
def write_to_google_sheet(excel_file_path, sheet_url, worksheet_name, auth_file, start_row=2):
    ...
    try:
        ...
        print(f"❌ 错误: 无法读取文件 {excel_file_path}")
        return False
    except Exception as e:
        print(f"❌ 读取 Excel 文件失败。错误: {e}")
        import traceback
        traceback.print_exc()
        return False
    ...
    try:
        ...
        # 5. 智能清空数据：只删除当年数据，保留往年数据
        print(f"正在处理 Google Sheets 中的数据...")
        try:
            # 获取当前年份
            ...
            if len(all_values) > 1:  # 如果有数据行（不只是表头）
                # A列（索引0）为"参考日期"，格式为YYYYMMDD
                header = all_values[0]  # 保存表头
                data_rows = all_values[1:]  # 数据行
                
                # 保留往年的数据
                previous_years_data = []
                for row in data_rows:
                    if len(row) > 0:  # 确保行不为空
                        ref_date = row[0]  # A列：参考日期
                        
                        # 从YYYYMMDD格式中提取年份
                        if ref_date and len(str(ref_date)) >= 4:
                            try:
                                # 提取前4位作为年份
                                year = int(str(ref_date)[:4])
                                
                                # 如果不是当年的数据，则保留
                                if year != current_year:
                                    previous_years_data.append(row)
                ...
                if previous_years_data:
                    prev_end_row = start_row + len(previous_years_data) - 1
                    prev_range = f"A{start_row}:{end_col_letter}{prev_end_row}"
                    worksheet.update(range_name=prev_range, values=previous_years_data)
                    print(f"✅ 已恢复 {len(previous_years_data)} 行往年数据")
                    ...
                    print(f"ℹ️ 没有往年数据需要保留")
                    
        except Exception as e:
            print(f"⚠️ 处理旧数据时出现警告: {e}，将采用完全覆盖模式...")
        
        # 6. 写入当年新数据
        print(f"正在将当年数据写入 Google Sheets: {worksheet_name}，范围: {range_name}")
        worksheet.update(range_name=range_name, values=data_to_write)
        
        print(f"✅ 成功将 {num_rows} 行当年数据写入 Google Sheet: {worksheet_name}（从第 {start_row} 行开始）")
        return True
